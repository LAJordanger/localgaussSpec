###----------------------------------------------------------------###
##  The EuStockMarkets-example from P2_fig_S3.1, focusing on the DAX-
##  and CAC-components of the logreturns.

##  This script generates a plot that combines heatmap-based plot for
##  the local Gaussian autospectra of the DAX- and CAC-components with
##  the corresponding distance-based plots.  The point of interest in
##  this plot is the effect on the estimated local Gaussian
##  cross-spectra as the point v varies along the diagonal.

###----------------------------------------------------------------###

##  In order for this script to work, it is necessary that the script
## '2_Data.R' (for P2_fig_07) has been used first.
...
.save_file_name <- "P2_fig_S3.1.pdf"

##  Select what kind of plot that should be produced.

##  Need converter from names to the (Y1,Y2,Y3,Y4)-setting used
##  internally.

..selected_margins <- c("DAX", "CAC")

..margins <- structure(
    .Names = colnames(EuStockMarkets),
    .Data = sprintf("Y%s",
                    seq_along(colnames(EuStockMarkets))))

##  DAX  SMI  CAC FTSE 
## "Y1" "Y2" "Y3" "Y4" 

##  For this script, only focus on the selected margins.

..margins <- ..margins[..selected_margins]

##  Initiate the list to be used for the plots.
...
for (..margin_name in names(..margins)) {
    ...
    ##  Define the 'input'-list that specifies the content of the
    ##  plot.  Some of the information in this list is redundant for
    ##  the present plot, but it is necessary to update the
    ##  plot-function before those parts can be removed from the list
    ##  below.
    ...
    ##  This part deals with the distance-based plot.  Note: The
...
################################################################################
    ...
    heatmap_plot_list[[..margin_name]] <- list(
...
for (.i in seq_along(heatmap_plot_list)) {
    print(heatmap_plot_list[[.i]]$heatmap  +
          ggtitle(label = NULL) +
    ...
    print(heatmap_plot_list[[.i]]$distance +
          ggtitle(label = NULL),
          vp = viewport(
