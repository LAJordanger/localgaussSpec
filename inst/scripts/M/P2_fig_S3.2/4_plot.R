###----------------------------------------------------------------###
##  The EuStockMarkets-example from P2_fig_S3.2, focusing on the DAX-
...
if (.plot_type  == "Cartesian") {
    ...
    .save_file_name <- "P2_fig_07.pdf"
} else {
    ..S_type_pair <- c("LS_c_amplitude", "LS_c_phase")
    .save_file_name <- "P2_fig_S3.2.pdf"
}
...
#####------------------------------------------------------------#####
    ...
    heatmap_plot <- LG_plot_helper(
        ...
        input_curlicues = list(
            ...
            ## ,
            ## title = list(
            ##     label = "Heatmap for the local Gaussian crosspectrum",
            ##     element_text = list(
            ##         size = 8))
        ))
